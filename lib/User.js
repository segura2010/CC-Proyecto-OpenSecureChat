		...
		this.getByUsername(user.username, function(err, users){
			if(users.length == 0)
			{
				c.insert({
					...
					// Save Private Keys to validate messages (is an index)
					// Then, user will send a message and his private key (encrypted), we will check if it is correct (like cookie)
					r.set("userpk:"+ user.privKey, newuser._id);
					cb(null, newuser);
				});
			}
			else
